...
export const getAllDeliveryMan = async (page, pageLimit,search) => {
    try {
        const response = await API.get(`deliveryMan?pageCount=${page}&pageLimit=${pageLimit}&isVerified=true`);
...
export const getPendingDeliveryMan = async (page, pageLimit,search) => {
    try {
        const response = await API.get(`deliveryMan?pageCount=${page}&pageLimit=${pageLimit}&isVerified=false${search}`);
        if (response.status === 200) {
            return { status: true, data: response.data.data }
        } else {
            console.log('API error', response.message)
...
export const getDeliveryManDocument = async (page, pageLimit) => {
    try {
        const response = await API.get(`/deliveryman/documents?pageCount=${page}&pageLimit=${pageLimit}`);
        if (response.status === 200) {
            return { status: true, data: response.data.data }
        } else {
            console.log('API error', response.message)
...
export const getDeliveryManLocation = async (page, pageLimit) => {
    try {
        const response = await API.get(`/deliveryman/locations?pageCount=${page}&pageLimit=${pageLimit}`);
        if (response.status === 200) {
            return { status: true, data: response.data.data }
        } else {
            console.log('API error', response.message)
...
export const updateDocumentStatus = async (data) => {
    try {
        const response = await API.patch(`/deliveryman/updateVerificationStatus`, data);
