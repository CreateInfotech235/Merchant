...
export const getScbscriptionUsers = async (page, pageLimit) => {
    try {
        const response = await API.get(`/users?isSubscribed=true&pageCount=${page}&pageLimit=${pageLimit}`);
        console.log('res',response)
        if (response.status === 200) {
            return { status: true, data: response.data.data }
        } else {
...
export const getPendingWithdraw = async (page, pageLimit) => {
    try {
        const response = await API.get(`/users/withdraw-history?transactionStatus=PENDING&pageCount=${page}&pageLimit=${pageLimit}`);
        console.log('res',response)
        if (response.status === 200) {
            return { status: true, data: response.data.data }
        } else {
...
export const getApprovedWithdraw = async (page, pageLimit) => {
    try {
        const response = await API.get(`/users/withdraw-history?transactionStatus=APPROVED&pageCount=${page}&pageLimit=${pageLimit}`);
        console.log('res',response)
        if (response.status === 200) {
            return { status: true, data: response.data.data }
        } else {
...
export const getRejectWithdraw = async (page, pageLimit) => {
    try {
        const response = await API.get(`/users/withdraw-history?transactionStatus=REJECT&pageCount=${page}&pageLimit=${pageLimit}`);
        console.log('res',response)
        if (response.status === 200) {
            return { status: true, data: response.data.data }
        } else {
